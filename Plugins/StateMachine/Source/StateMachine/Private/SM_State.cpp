	...
	const TArray<USM_InputAtom*>& DataSource, int32 DataIndex /*= 0*/, int32 RemainingSteps/* = -1*/)
{
	...
	// If we re still runnning, see where our branches lead.
	if (RemainingSteps && !bMustEndNow)
	{
		...
		{
			// this could be a check. There shouldnt be null branches in the list.
		...
		// We didnt find any acceptable branch, so we must end on this state unless we re told to loop.
		if (bLoopByDefault)
		{
